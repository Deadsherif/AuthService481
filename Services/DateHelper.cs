    ...
    public static class DateHelper
    {
        public static async Task<DateTime> GetCurrentDateFromWebAsync()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                try
                {
                    HttpResponseMessage response = await client.GetAsync("https://aisenseapi.com/services/v1/datetime").ConfigureAwait(false);
                ...
                catch
                {
                    ...
                    try
                    {
                        HttpResponseMessage response = await client.GetAsync("https://api.ipgeolocation.io/timezone?apiKey=d27b8d6f8d6a4a899999d2e61aa6bab6&tz=UTC").ConfigureAwait(false);
                        response.EnsureSuccessStatusCode(); // Throws exception if status code is not 2xx
                        string content = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
